...
<script>
  $('input:radio[name=Ambito]').click(function() {
    ...
    var sx=$("input[name='Sexo']:checked").val();
      ...
      $.getJSON("act_graf_gru_estab/"+sx+"/"+edv+"/"+amv+"/"+mrv+"/"+esv,function(data){
        graf_gru_est(data);
        switch(amv)
        {
            case 'R':
