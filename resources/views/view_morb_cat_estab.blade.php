...
$("#edad").change(function(event){
  ...
  $.getJSON("act_morb_cat_estab/"+sx+"/"+event.target.value+"/"+mcr+"/"+esv,function(data){
    var linea="<tbody>";
    $.each(data, function(){
      linea+="<tr>";
    ...
    $('h2').text("Morbilidad 2015 por Categorias CIE-10 "+"Micro Red "+nommic);
...
$("#micro").change(function(event){
  ...
  $.getJSON("act_morb_cat_estab/"+sx+"/"+edv+"/"+mcr+"/"+esv,function(data){
    var linea="<tbody>";
    $.each(data, function(){
      linea+="<tr>";
      linea+="<td>"+this.categoria+"</td>";
