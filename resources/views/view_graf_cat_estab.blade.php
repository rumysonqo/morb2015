...
<script>
  $('input:radio[name=Ambito]').click(function() {
    ...
    var sx=$("input[name='Sexo']:checked").val();
      ...
      $.getJSON("act_graf_cat_estab/"+sx+"/"+edv+"/"+amv+"/"+mrv+"/"+esv,function(data){
        graf_cat_est(data);
        switch(amv)
        {
            case 'R':
...
<script>
    function graf_cat_est(datos) {
        ...
        var chartData = [];
                ...
                {
                    chartData.push({"categoria":n['categoria'],
                    "total":n['total'],
                ...
                if(RESTO>0)
                {
                    ...
                    chartData.push({"categoria":"Otros",
                    "total":RESTO,
